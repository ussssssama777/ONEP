...
async function addDistribution() {
  ...
  const qt_stock   = parseInt(document.getElementById('f_qt_stock').value);
  ...
  // ✅ Bloquer si stock <= 5
  if (qt_stock <= 5) {
    alert(`Stock insuffisant (${qt_stock}). Distribution impossible pour cette fourniture.`);
    return;
  }
  ...
  try {
    const res  = await fetch('/api/distribution', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ matricule, code_f, date, qte, recuperePar,nom,prenom })
    });
    const data = await res.json();
    ...
    a.href     = url;
    a.download = `distribution_${matricule}_${date}.pdf`;
    a.click();
    window.URL.revokeObjectURL(url);

    ['dist_matricule', 'dist_code_f', 'dist_date', 'dist_qte', 'recuperePar',
     'p_matricule', 'p_nom', 'p_prenom', 'p_code_s', 'p_libelle',
